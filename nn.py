...
def train(X, Y, W1, W2):
    ...
    l1 = np.dot(X, W1)
    print("l1: ", l1.shape)
    l2 = np.dot(sigmoid(l1), W2).reshape()
    print("l2: ", l2.shape)
    ...
    #Update weights
    print("W1: ", np.dot(sigmoid(X), loss1).shape)
    W1 = W1 + alpha * np.dot(sigmoid(X), loss1)
    print("W2: ", np.dot(sigmoid(l1), loss2).shape)
    W2 = W2 + alpha * np.dot(sigmoid(l1), loss2)
